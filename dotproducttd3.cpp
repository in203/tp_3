...
double dot( std::vector<double>& u, std::vector<double>& v )
{
  ...
  double scal = 0.;
//# pragma omp parallel for reduction(+:scal)//on peut paralleliser ici
...
#pragma omp parallel for schedule(dynamic)// j'ai utilisé dynamic car si un thread a termine le traitement il aura une autre tache a faire
  ...
  double ref = result[0];
  double sum = 0;
  //#pragma omp parallel for reduction(+:sum)//j'ai parallelisé la boucle avec reduction pour le calcul de la somme 
